...
WORKDIR /src
COPY ["MovieLambdaFunction.csproj", "MovieLambdaFunction/"]
RUN dotnet restore "MovieLambdaFunction/MovieLambdaFunction.csproj"

WORKDIR "/src/MovieLambdaFunction"
COPY . .
RUN dotnet build "MovieLambdaFunction.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MovieLambdaFunction.csproj" \
...
Execute unit tests
```
cd "MovieLambdaFunction/test/MovieLambdaFunction.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "MovieLambdaFunction/src/MovieLambdaFunction"
